...
#### POST /api/entry/bulk-delete

**New in v1.3**: Deletes multiple entries in a single transaction. This is significantly faster than issuing individual DELETE requests.
...
*Note: If partial failures occur (e.g., file locked on disk), the `errors` array may contain details, but the DB records will be removed transactionally.*
